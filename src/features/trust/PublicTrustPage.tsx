    ...
    <div className="stage min-h-screen">
      <div className="mx-auto max-w-5xl px-5 py-8 sm:px-8">
        <header className="mb-10 flex flex-wrap items-center justify-between gap-3">
          ...
          <Tag tone="accent">
            <Eye size={11} /> {tr('Trust Center, lecture seule', 'Trust Center, read-only')}
          </Tag>
        </header>
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="space-y-6">
            <div>
              <div className="flex flex-wrap items-center gap-2 text-xs text-accent">
                {tr('Posture réglementaire déclarée', 'Declared regulatory posture')}
                {data.is_demo ? <Tag>{tr('Démonstration', 'Demo')}</Tag> : null}
              </div>
              <h1 className="mt-2 text-3xl font-semibold tracking-tight text-ink">{data.name}</h1>
              <p className="mt-2 max-w-2xl text-sm leading-relaxed text-ink-2">
                {tr(
                  `Textes européens applicables et avancement déclaré sur les exigences qu'ils imposent. Mise à jour le ${formatDate(data.updated_at)}.`,
                  `Applicable European texts and declared progress on the requirements they set. Updated on ${formatDate(data.updated_at)}.`,
                )}
              </p>
            </div>
            {data.snapshot ? (
              ...
              <EmptyState title={tr("Aucune donnée publiée pour l'instant", 'No data published yet')}>{tr("Le cadrage de cette entité n'est pas encore terminé.", "This entity's scoping is not finished yet.")}</EmptyState>
            )}
            <Disclaimer />
          </motion.div>
        )}
