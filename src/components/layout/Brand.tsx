 ...
 * est dedans. À l'intérieur, les cinq textes (un nœud chacun, à sa couleur)
...
const MARK_GEOMETRY = {
  ...
  nodes: [
    { x: 12, y: 6.5, reg: 'rgpd' },
    { x: 17.2, y: 10.3, reg: 'nis2' },
    { x: 15.2, y: 16.4, reg: 'dora' },
    { x: 8.8, y: 16.4, reg: 'cra' },
    { x: 6.8, y: 10.3, reg: 'aiact' },
  ],
  hub: { x: 12, y: 12 },
} as const
...
export function Mark({ size = 18, className, mono = false }: { size?: number; className?: string; mono?: boolean }) {
  ...
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" className={cn('shrink-0', className)} role="img" aria-label="Scopeo">
      ...
      {g.nodes.map((n) => (
        <circle key={n.reg} cx={n.x} cy={n.y} r={1.7} fill={mono ? 'currentColor' : `var(--c-${n.reg})`} />
      ))}
    ...
    <span className={cn('flex min-w-0 items-center gap-2.5', className)}>
      ...
      {collapsed ? null : (
        <span className={cn('min-w-0 leading-tight', compact && 'hidden sm:block')}>
          ...
          <span className="block truncate font-mono text-[10px] uppercase tracking-[0.12em] text-ink-4">{tr('Cadrage réglementaire', 'Regulatory scoping')}</span>
        </span>
      )}
    </span>
  )
